...
> 상태: **Phase 2 구현 완료 후 조정됨 (2026-07-08)**. 최초 설계는 7개 섹션이었으나, 실제 토큰
> 사용량을 실측한 뒤 다음과 같이 조정했다:
> - **AI/CV 뉴스 + 반도체/임베디드 뉴스 → "AI / 임베디드 뉴스"로 통합** (검색 1회로 축소, 평균 4건).
>   AI가 CV의 상위 분류이고, 사용자 전공(Embedded)에 더 잘 맞는 임베디드 뉴스에 무게를 둠.
> - **취업 정보(JOB_MARKET)는 일시 중단** — 파이프라인 호출/이메일 렌더링 모두 제외하되, 코드는
>   `lib/modules/jobMarket`에 보존(향후 재사용 대비).
> - **코딩테스트는 프로그래머스로 한정** (LeetCode/백준 등 제외).
> 아래 본문의 "7개 섹션", 개별 SEMICONDUCTOR/JOB_MARKET 관련 서술, 토큰/비용 추정치는 이 조정
> 이전의 최초 설계 스냅샷이다 — 현재 구현 기준의 원천은 `src/` 코드이며, 이 문서는 설계 배경
> 설명용으로 유지한다.
>
